...
@if (Session::has('cart') != null)
    <ul id="cart-sidebar" class="mini-products-list">
        @foreach (Session::get('cart')->products as $item)
            <li class="item odd">
                <a href="{{route('detail',['id'=>$item['productInfo']->id])}}" title="Product title here" class="product-image">
                    <img src="{{ asset("storage/".$item['productInfo']->image_url) }}" alt="html Template" width="65">
                </a>
                <div class="product-details">
                    ...
                    <strong>{{ $item['quantity'] }}</strong> x <span class="price">{{ number_format($item['productInfo']->price) }}Đ</span>
                </div>
            </li>
        @endforeach
    </ul>
