<div class="slider-area slider-area-3">
    <div class="slider-carousel-active owl-carousel arrow-style-2">
        <!-- single-slider-wrapper -->
        <div class="single-slider-wrapper slider-3-1">
            <div class="container">
                <div class="row">
                    <div class="col-md-9">
                        <div class="slider-content slider-content-1 slider-animated-1">
                            <h3>SALE<strong> -40%</strong> </h3> <br/>
                            <h2><strong>NEW</strong> COLLECTION</h2> <br/>
                            <h4>WOMEN'S<strong> SPORTS</strong> </h4> <br/>
                            <div class="button-1 hover-button-1">
                                <a href="single-product.html">SHOP IT</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- single-slider-wrapper -->
        <div class="single-slider-wrapper slider-3-2">
            <div class="container">
                <div class="row">
                    <div class="col-md-9">
                        <div class="slider-content slider-content-2 slider-animated-1">
                            <h3>SALE UP TO OFF ALL </h3> <br/>
                            <h2><strong>50% </strong><span> BACK PACK</span><span>&<b> JACKETS</b></span></h2> <br/>
                            <div class="button-1 hover-btn-2">
                                <a href="single-product.html">SHOP IT</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
